  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        ...
        <Link href="/admin/blogs/new" className="px-5 py-3 bg-coral-500 hover:bg-coral-600 text-white rounded-xl font-bold shadow-lg shadow-coral-500/30">+ Add New Blog</Link>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white p-12 rounded-2xl border border-ink-900/10 text-center">
          ...
          <p className="text-ink-900/70 mb-4">No blogs yet.</p>
          <Link href="/admin/blogs/new" className="inline-block px-5 py-3 bg-coral-500 text-white rounded-xl font-bold">+ Create Blog</Link>
        </div>
      ) : (
          ...
          {list.map((b: any) => (
            <div key={b.id} className="p-5 flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <Link href={`/admin/blogs/${b.id}`} className="px-3 py-1.5 bg-coral-500 text-white rounded-lg text-sm font-bold">Edit</Link>
